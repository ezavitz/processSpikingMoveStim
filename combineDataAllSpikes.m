...
function [sTrain, onsetInds, StimFile, clusters] = combineDataAllSpikes(rootDir, prefixes)
...
for a = 1:length(prefixes)
    ...
    for f = 1:length(fileList)
        ...
        spike.elec = spike.cluster;
        for cID = 1:length(clust.ID)
            spike.elec(spike.elec == clust.ID(cID)) = clust.ch(cID);
        end
        
        
        % make that spike train
        getUnits = ismember(spike.cluster, clust.index(clust.isUnit)); % get all units
        sTrain{f}{a} = buildSpikeTrain(spike.ms(getUnits), spike.elec(getUnits), 1000, clust.index(clust.isUnit)); 
